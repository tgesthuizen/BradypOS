...
struct tcb_t *get_thread_by_index(unsigned idx);
unsigned get_thread_count();
unsigned get_tcb_index(struct tcb_t *tcb);
...
void delete_thread(struct tcb_t *thread);
void write_utcb(struct tcb_t *thread);
